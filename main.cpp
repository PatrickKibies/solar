...
int main(int argc, char **argv) {
 ...
 double f_Force = f_grav_force(f_mSun,f_mEarth,f_r);
  ...
  vec_initial_velocity(1) = 0.1; //ca. max speed of earth in perihel
  
 
  
  vec_Earth_old_Position.set_size(3);
  vec_Earth_old_Position.zeros();
  vec_Earth_old_Position(0) = 0.98328 * 149597870.700; //Distance Sun earth, metres
  
  vec_Earth_current_Position.set_size(3);
  vec_Earth_current_Position.zeros();
  vec_Earth_current_Position = vec_positionVelocity(vec_Earth_old_Position, vec_initial_velocity, 1.0);
  ...
  for(int i = 1; i<=100; i++){
    ...
    vec_Earth_current_Position = vec_verlet_step(vec_temp, vec_Earth_current_Position, -vec_accel, 1.0);

    
   std::cout << "Position:" << vec_Earth_current_Position << "vec_accel" << vec_accel << std::endl;
